...
qede_tx_mapped(qede_tx_ring_t *tx_ring, mblk_t *mp, qede_tx_pktinfo_t *pktinfo)
{
		...
		if (!is_premapped) {
			...
			if (ddi_dma_addr_bind_handle(dmah_entry->dma_handle,
				...
				qede_info(tx_ring->qede, "addr_bind() failed for "
				    "handle %p, len %d", dmah_entry->dma_handle,
				    mblen);

				if ((pktinfo->total_len < tx_ring->tx_buf_size) &&
				    (!pktinfo->use_lso)) {
					qede_info(tx_ring->qede, "Falling back to bcopy");
					status = XMIT_FALLBACK_BCOPY;
				} else {
	...
	if (ecore_chain_get_elem_left(&tx_ring->tx_bd_ring) <
	    nbd) {
		/*
		 * See if we can copy the packet,
		 * else sleep the tx queue for more
		 * descriptors 
		 */
		if ((pktinfo->total_len > tx_ring->tx_buf_size) ||
		    (pktinfo->use_lso)) {
...
err_map:
	...
	if (head != NULL)
		qede_put_dmah_entries(tx_ring, dmah_entry);
...
do_pullup:
	...
	if (xmit_mode == USE_DMA_BIND) {
		...
		if (status == XMIT_DONE) {
			...
			else if(pktinfo.total_len > 1514)
				tx_ring->tx_jumbo_pkt_count++;
			tx_ring->tx_mapped_pkts++;
			goto exit;
		} else if (status == XMIT_TOO_MANY_COOKIES && !force_pullup) {
			xmit_mode = USE_PULLUP;
		} else {
			status = XMIT_FAILED;
			goto exit;
